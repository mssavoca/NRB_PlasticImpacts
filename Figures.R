...
final_plot <- annotate_figure(
  combined_plot,
  top = text_grob("Ingestion effects", color = "black", face = "bold", size = 26)
)

final_plot

# Save to PDF
ggsave("Plastic_Ingestion_Effects_final_nolabels_v2.pdf", final_plot, width = 10, height = 10)
...
ggsave("Plastic_Ingestion_Effects_map_with_shape_MargDens_v2.pdf", map_with_densities, width = 14, height = 12)
...
entangle_filtered <- df_deduped_entangle %>%
...
plot_A <- ggplot(entangle_filtered, aes(x = Decade, fill = Effect_general)) +
  ...
  scale_y_continuous(
    breaks = scales::pretty_breaks(),        # Choose sensible breaks
    labels = scales::label_number(accuracy = 1)  # Ensure integer labels only
  ) +
  theme_minimal(base_size = 20) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
final_plot <- annotate_figure(
  combined_plot,
  top = text_grob("Entanglement effects", color = "black", face = "bold", size = 26)
)
...
# Save to PDF
ggsave("Plastic_Entanglement_Effects_final_nolabels_v2.pdf", final_plot, width = 10, height = 10)
